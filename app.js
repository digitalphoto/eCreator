...
function cropFirstRatio(){
  const [w,h]=photoSize();
  return Math.max(.2,Number(w||35)/Math.max(.2,Number(h||45)));
}
function cropFirstLayout(){
  const stage=$('cropFirstStage'),box=$('cropFirstBox');
  if(!stage||!box)return null;
  const sw=Math.max(1,stage.clientWidth),sh=Math.max(1,stage.clientHeight);
  const ratio=cropFirstRatio(),pad=Math.max(18,Math.min(sw,sh)*.07);
  let bw=Math.max(40,sw-pad*2),bh=bw/ratio;
  if(bh>sh-pad*2){bh=Math.max(40,sh-pad*2);bw=bh*ratio}
  const left=(sw-bw)/2,top=(sh-bh)/2;
  Object.assign(box.style,{left:left+'px',top:top+'px',width:bw+'px',height:bh+'px'});
  const [w,h]=photoSize();
  if($('cropRatioLabel'))$('cropRatioLabel').textContent=`${Number(w).toFixed(2).replace(/\\.00$/,'')} × ${Number(h).toFixed(2).replace(/\\.00$/,'')} mm`;
  return {sw,sh,bw,bh,left,top};
}
function drawCropFirst(){
  const st=cropFirstState,L=cropFirstLayout(),cv=$('cropFirstCanvas');
  if(!L||!cv||!st.img)return;
  const dpr=Math.min(2,window.devicePixelRatio||1);
  cv.width=Math.max(1,Math.round(L.sw*dpr));
  cv.height=Math.max(1,Math.round(L.sh*dpr));
  const ctx=cv.getContext('2d');
  ctx.setTransform(dpr,0,0,dpr,0,0);
  ctx.clearRect(0,0,L.sw,L.sh);
  const iw=Math.max(1,st.img.naturalWidth||st.img.width||1);
  const ih=Math.max(1,st.img.naturalHeight||st.img.height||1);
  const scale=Math.max(.0001,st.baseScale*st.scale);
...
function resetCropFirstPosition(){
  const st=cropFirstState,L=cropFirstLayout();
  if(!st.img||!L)return;
  const iw=Math.max(1,st.img.naturalWidth||st.img.width||1);
  const ih=Math.max(1,st.img.naturalHeight||st.img.height||1);
  const cover=Math.max(L.bw/iw,L.bh/ih);
  st.baseScale=cover;st.scale=1;
  const dw=iw*cover,dh=ih*cover;
  st.x=L.left+(L.bw-dw)/2;
  st.y=L.top+(L.bh-dh)/2;
  if($('cropZoom'))$('cropZoom').value=100;
  if($('cropZoomValue'))$('cropZoomValue').textContent='100%';
  drawCropFirst();
}
async function openCropFirst(file){
  const data=await fileToDataURL(file);
  const img=await imageFromUrl(data);
  cropFirstState.file=file;
  cropFirstState.img=img;
  $('cropFirstOverlay').classList.remove('hidden');
  $('cropFirstOverlay').setAttribute('aria-hidden','false');
  requestAnimationFrame(()=>requestAnimationFrame(()=>resetCropFirstPosition()));
}
...
async function commitCropFirst(){
  const st=cropFirstState,L=cropFirstLayout();
  if(!st.img||!L)return;
  const [mmW,mmH]=photoSize();
  const dpi=Math.max(150,Math.min(1200,Number(outputDpi()||300)));
  const outW=Math.max(200,Math.round(mmW/25.4*dpi));
  const outH=Math.max(200,Math.round(mmH/25.4*dpi));
  ...
  ctx.drawImage(st.img,sx,sy,sw,sh,0,0,outW,outH);
  const blob=await new Promise((resolve,reject)=>c.toBlob(b=>b?resolve(b):reject(new Error('Crop export failed')),'image/jpeg',.96));
  const croppedFile=new File([blob],`cropped_${st.file?.name||'photo.jpg'}`,{type:'image/jpeg'});
  closeCropFirst();
  cropFirstState.bypass=true;
  const dt=new DataTransfer();
  dt.items.add(croppedFile);
  cropPhotoInput.files=dt.files;
  cropPhotoInput.dispatchEvent(new Event('change',{bubbles:true}));
  setTimeout(()=>{cropFirstState.bypass=false;},0);
  status('Crop confirmed. Main Studio is ready for design tools.','ok');
}
const cropPhotoInput=$('photoInput');
cropPhotoInput?.addEventListener('change',e=>{
...
$('cropChooseAnotherBtn')?.addEventListener('click',()=>{closeCropFirst();cropPhotoInput.value='';cropPhotoInput.click()});
