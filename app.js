...
let faceDetectorPromise=null,lastFaceDetection=null,autoCreateBusy=false;
...
$('photoInput').addEventListener('change',async e=>{
  ...
  try{
    ...
    if($('autoCreateUpload')?.checked)await runAutoPassportWorkflow({showResult:true});
    else detectMainFace().catch(()=>{});
...
/* ===== v2.7 Real Face Detection + One-Click Auto Create ===== */
const FACE_WASM_URL='https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.22/wasm';
...
async function getFaceDetector(){
  ...
  faceDetectorPromise=(async()=>{
    ...
    return vision.FaceDetector.createFromOptions(fileset,{baseOptions:{modelAssetPath:FACE_MODEL_URL},runningMode:'IMAGE',minDetectionConfidence:.45});
  })();
  return faceDetectorPromise;
}
function setFaceDetectInfo(text,type=''){
  const el=$('faceDetectInfo');if(el){el.textContent=text;el.classList.remove('ok','warn','error');if(type)el.classList.add(type)}
}
function pickBestFace(detections=[]){
  if(!detections.length)return null;
  return detections.slice().sort((a,b)=>{
    const A=(a.boundingBox?.width||0)*(a.boundingBox?.height||0),B=(b.boundingBox?.width||0)*(b.boundingBox?.height||0);
    return B-A;
  })[0];
}
async function detectMainFace(){
  if(!originalImageEl&&!sourceDataUrl)return null;
  setFaceDetectInfo('Detecting face…');if($('faceDetectBadge'))$('faceDetectBadge').textContent='Detecting…';
  try{
    const detector=await getFaceDetector(),img=originalImageEl||await imageFromUrl(sourceDataUrl),res=detector.detect(img),best=pickBestFace(res?.detections||[]);
    if(!best?.boundingBox)throw new Error('No clear face detected');
    lastFaceDetection={box:best.boundingBox,score:best.categories?.[0]?.score||0,inputW:img.naturalWidth||img.width,inputH:img.naturalHeight||img.height};
    const pct=Math.round((lastFaceDetection.score||0)*100);setFaceDetectInfo(`Face detected${pct?` • confidence ${pct}%`:''}. Auto passport positioning is ready.`,'ok');if($('faceDetectBadge'))$('faceDetectBadge').textContent='Face ✓';return lastFaceDetection;
  }catch(err){
    console.warn('Face detector fallback:',err);lastFaceDetection=null;setFaceDetectInfo('Face AI unavailable or no clear face found. Using safe automatic composition fallback.','warn');if($('faceDetectBadge'))$('faceDetectBadge').textContent='Fallback';return null;
  }
}
function faceFitTargets(){
  const mode=$('faceFitMode')?.value||'standard';
  if(mode==='tight')return {faceH:.37,faceCY:.30};
  if(mode==='relaxed')return {faceH:.30,faceCY:.32};
  return {faceH:.34,faceCY:.31};
}
function applyDetectedFaceComposition(det=lastFaceDetection){
  if(!fabricCanvas||!subjectObj||!det?.box)return false;
  setMasterLock(false);
  const W=fabricCanvas.width,H=fabricCanvas.height,b=det.box,t=faceFitTargets();
  const srcW=det.inputW||subjectObj.width,srcH=det.inputH||subjectObj.height;
  const localFaceH=(b.height||1)*(subjectObj.height/Math.max(1,srcH));
  const localFaceCX=(b.originX+b.width/2)*(subjectObj.width/Math.max(1,srcW));
  const localFaceCY=(b.originY+b.height/2)*(subjectObj.height/Math.max(1,srcH));
  const s=(H*t.faceH)/Math.max(1,localFaceH);
  subjectObj.set({originX:'left',originY:'top',scaleX:s,scaleY:s,angle:0,flipX:false,flipY:false});
  subjectObj.left=W/2-localFaceCX*s;
  subjectObj.top=H*t.faceCY-localFaceCY*s;
  subjectObj.setCoords();fabricCanvas.setActiveObject(subjectObj);fabricCanvas.requestRenderAll();syncPhotoBorder();syncFaceGuide();saveHistory();return true;
}
function safeFallbackCoverComposition(){
  if(!fabricCanvas||!subjectObj)return false;
  setMasterLock(false);
  const W=fabricCanvas.width,H=fabricCanvas.height;
  // IMPORTANT: fallback must fill the complete final passport frame.
  // Never place a small original-photo rectangle over the background.
  const iw=Math.max(1,subjectObj.width||1),ih=Math.max(1,subjectObj.height||1);
  ...
  subjectObj.setCoords();
  fabricCanvas.setActiveObject(subjectObj);
  fabricCanvas.requestRenderAll();
  syncBackgroundFrame();syncPhotoBorder();syncFaceGuide();saveHistory();
  setFaceDetectInfo('Face AI unavailable. Safe full-frame crop applied — no small inner photo box.','warn');
  if($('faceDetectBadge'))$('faceDetectBadge').textContent='Safe Crop';
  status('Face AI fallback: photo now fills the complete passport frame.','ok');
  return true;
}
async function smartAutoCompose(){
  const det=lastFaceDetection||await detectMainFace();
  if(det&&applyDetectedFaceComposition(det)){runQualityCheck(false);status('Face detected and passport crop/position applied automatically.','ok');return true}
  safeFallbackCoverComposition();runQualityCheck(false);return false;
}
async function showAutoResult(){
  await renderSheet();$('resultTabs').classList.remove('hidden');$('actions').classList.remove('hidden');$('backEditorBtn').classList.remove('hidden');document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.tab==='sheet'));$('previewStage').classList.add('hidden');$('sheetPreviewWrap').classList.remove('hidden');status('Auto passport design created. Use Back to Editor for manual fine-tuning.','ok');
}
async function runAutoPassportWorkflow({showResult=true}={}){
  if(autoCreateBusy||!fabricCanvas||!subjectObj)return;autoCreateBusy=true;
  try{
    setFaceDetectInfo('Analyzing photo and preparing passport composition…');
    const faceApplied=await smartAutoCompose();
    if($('removeBg')?.checked&&!removedBlob)await removeBackgroundNow();
    // BG removal replaces the subject bitmap. Preserve the detected transform;
    // if detection failed, enforce full-frame cover again to prevent an inner white photo box.
    if(!faceApplied)safeFallbackCoverComposition();
    syncBackgroundFrame();syncPhotoBorder();runQualityCheck(false);
    if(showResult)await showAutoResult();
  }catch(err){console.error(err);setFaceDetectInfo('Automatic create could not finish. Manual editor is still available.','error');status('Auto create could not finish. You can continue manually.','error')}
  finally{autoCreateBusy=false}
}
$('autoCreateNowBtn')?.addEventListener('click',()=>runAutoPassportWorkflow({showResult:true}));
$('faceFitMode')?.addEventListener('change',()=>{if(lastFaceDetection&&fabricCanvas){applyDetectedFaceComposition(lastFaceDetection);runQualityCheck(false)}});
window.addEventListener('load',()=>setTimeout(()=>getFaceDetector().catch(()=>{}),1800));
...
const _uiStateV27=uiState;uiState=function(){return {..._uiStateV27(),autoCreateUpload:$('autoCreateUpload')?.checked,faceFitMode:$('faceFitMode')?.value}};
...
/* ===== v3.0 Crop-First Studio Workflow ===== */
let cropFirstState={file:null,img:null,scale:1,baseScale:1,x:0,y:0,drag:false,lastX:0,lastY:0,bypass:false};

function cropFirstRatio(){
  const sz=photoSize(); return Math.max(.2,sz.w/Math.max(.2,sz.h));
}
function cropFirstLayout(){
  const stage=$('cropFirstStage'),box=$('cropFirstBox'); if(!stage||!box)return null;
  const sw=stage.clientWidth,sh=stage.clientHeight,ratio=cropFirstRatio(),pad=Math.max(18,Math.min(sw,sh)*.07);
  let bw=sw-pad*2,bh=bw/ratio;if(bh>sh-pad*2){bh=sh-pad*2;bw=bh*ratio}
  const left=(sw-bw)/2,top=(sh-bh)/2;
  Object.assign(box.style,{left:left+'px',top:top+'px',width:bw+'px',height:bh+'px'});
  const sz=photoSize();if($('cropRatioLabel'))$('cropRatioLabel').textContent=`${round2(sz.w)} × ${round2(sz.h)} mm`;
  return {sw,sh,bw,bh,left,top};
}
function drawCropFirst(){
  const st=cropFirstState,L=cropFirstLayout(),cv=$('cropFirstCanvas');if(!L||!cv||!st.img)return;
  const dpr=Math.min(2,window.devicePixelRatio||1);cv.width=Math.round(L.sw*dpr);cv.height=Math.round(L.sh*dpr);
  const ctx=cv.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,L.sw,L.sh);
  const iw=st.img.naturalWidth,ih=st.img.naturalHeight,scale=st.baseScale*st.scale,dw=iw*scale,dh=ih*scale;
  // constrain so crop box can never expose empty/transparent space
  const minX=L.left+L.bw-dw,maxX=L.left,minY=L.top+L.bh-dh,maxY=L.top;
  st.x=Math.min(maxX,Math.max(minX,st.x));st.y=Math.min(maxY,Math.max(minY,st.y));
  ctx.drawImage(st.img,st.x,st.y,dw,dh);
}
function resetCropFirstPosition(){
  const st=cropFirstState,L=cropFirstLayout();if(!st.img||!L)return;
  const cover=Math.max(L.bw/st.img.naturalWidth,L.bh/st.img.naturalHeight);
  st.baseScale=cover;st.scale=1;const dw=st.img.naturalWidth*cover,dh=st.img.naturalHeight*cover;
  st.x=L.left+(L.bw-dw)/2;st.y=L.top+(L.bh-dh)/2;if($('cropZoom'))$('cropZoom').value=100;if($('cropZoomValue'))$('cropZoomValue').textContent='100%';drawCropFirst();
}
async function openCropFirst(file){
  const data=await fileToDataURL(file),img=await loadImage(data);cropFirstState.file=file;cropFirstState.img=img;
  $('cropFirstOverlay').classList.remove('hidden');$('cropFirstOverlay').setAttribute('aria-hidden','false');
  requestAnimationFrame(()=>resetCropFirstPosition());
}
function closeCropFirst(){
  $('cropFirstOverlay')?.classList.add('hidden');$('cropFirstOverlay')?.setAttribute('aria-hidden','true');
}
async function commitCropFirst(){
  const st=cropFirstState,L=cropFirstLayout();if(!st.img||!L)return;
  const sz=photoSize(),dpi=Math.max(150,Math.min(1200,Number(currentDpi?.()||300))),outW=Math.max(200,Math.round(sz.w/25.4*dpi)),outH=Math.max(200,Math.round(sz.h/25.4*dpi));
  const c=document.createElement('canvas');c.width=outW;c.height=outH;const ctx=c.getContext('2d');
  const scale=st.baseScale*st.scale,dw=st.img.naturalWidth*scale,dh=st.img.naturalHeight*scale;
  const sx=(L.left-st.x)/scale,sy=(L.top-st.y)/scale,sw=L.bw/scale,sh=L.bh/scale;
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(st.img,sx,sy,sw,sh,0,0,outW,outH);
  const blob=await new Promise(r=>c.toBlob(r,'image/jpeg',.96));const croppedFile=new File([blob],`cropped_${st.file?.name||'photo.jpg'}`,{type:'image/jpeg'});
  closeCropFirst();cropFirstState.bypass=true;
  // Feed the confirmed crop into the existing, proven upload/editor pipeline.
  const dt=new DataTransfer();dt.items.add(croppedFile);photoInput.files=dt.files;photoInput.dispatchEvent(new Event('change',{bubbles:true}));
  setTimeout(()=>{cropFirstState.bypass=false;},0);
  status('Crop confirmed. Main Studio is ready for background, retouch, suit, border and print design.','ok');
}
photoInput?.addEventListener('change',e=>{
  if(cropFirstState.bypass)return;
  const f=e.target.files?.[0];if(!f)return;
  e.stopImmediatePropagation();e.preventDefault();openCropFirst(f).catch(err=>status('Could not open crop editor: '+err.message,'error'));
},true);

$('cropZoom')?.addEventListener('input',e=>{cropFirstState.scale=Number(e.target.value)/100;$('cropZoomValue').textContent=e.target.value+'%';drawCropFirst()});
$('cropCenterBtn')?.addEventListener('click',resetCropFirstPosition);
document.querySelectorAll('[data-crop-nudge]').forEach(b=>b.addEventListener('click',()=>{const d=b.dataset.cropNudge,n=8;if(d==='left')cropFirstState.x-=n;if(d==='right')cropFirstState.x+=n;if(d==='up')cropFirstState.y-=n;if(d==='down')cropFirstState.y+=n;drawCropFirst()}));
$('cropContinueBtn')?.addEventListener('click',commitCropFirst);
$('cropChooseAnotherBtn')?.addEventListener('click',()=>{closeCropFirst();photoInput.value='';photoInput.click()});
const cropStage=$('cropFirstStage');
cropStage?.addEventListener('pointerdown',e=>{cropFirstState.drag=true;cropFirstState.lastX=e.clientX;cropFirstState.lastY=e.clientY;cropStage.setPointerCapture?.(e.pointerId)});
cropStage?.addEventListener('pointermove',e=>{if(!cropFirstState.drag)return;cropFirstState.x+=e.clientX-cropFirstState.lastX;cropFirstState.y+=e.clientY-cropFirstState.lastY;cropFirstState.lastX=e.clientX;cropFirstState.lastY=e.clientY;drawCropFirst()});
cropStage?.addEventListener('pointerup',()=>cropFirstState.drag=false);cropStage?.addEventListener('pointercancel',()=>cropFirstState.drag=false);
window.addEventListener('resize',()=>{if(!$('cropFirstOverlay')?.classList.contains('hidden'))requestAnimationFrame(drawCropFirst)});
