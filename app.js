...
function updateInfo(){const [w,h]=photoSize(),dpi=outputDpi(),pxW=mmToPx(w,dpi),pxH=mmToPx(h,dpi);$('previewInfo').textContent=`${w} × ${h} mm • ${dpi} DPI • ${pxW} × ${pxH} px`;$('pixelInfo').textContent=`${pxW} × ${pxH} px`}
...
function uiState(){return {nepalPreset:$('nepalPreset').value,photoSize:$('photoSize').value,customWidth:$('customWidth').value,customHeight:$('customHeight').value,bgColor:$('bgColor').value,bgHex:$('bgHex').value,autoEnhance:$('autoEnhance').checked,brightness:$('brightness').value,contrast:$('contrast').value,saturation:$('saturation').value,outputDpi:$('outputDpi').value}}
function applyUiState(s={}){restoringHistory=true;Object.entries(s).forEach(([k,v])=>{const el=$(k);if(!el)return;if(el.type==='checkbox')el.checked=!!v;else el.value=v});$('customSizeBox').classList.toggle('hidden',$('photoSize').value!=='custom');updateInfo();restoringHistory=false}
...
function exportSingle(type='image/png'){if(!fabricCanvas)return null;const [mmW]=photoSize(),targetW=mmToPx(mmW,outputDpi()),mult=targetW/fabricCanvas.width;const active=fabricCanvas.getActiveObject();fabricCanvas.discardActiveObject();fabricCanvas.requestRenderAll();const out=fabricCanvas.toDataURL({format:type==='image/jpeg'?'jpeg':'png',quality:1,multiplier:mult,enableRetinaScaling:false});if(active){fabricCanvas.setActiveObject(active);fabricCanvas.requestRenderAll()}return out}
